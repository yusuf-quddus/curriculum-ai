...
export function Button({ children, className = '', ...props }) {
  return (
    <button
      className={
        'px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded ' +
        className
      }
      {...props}
    >
...
export default function UploadPage() {
  ...
  const handleProcess = async () => {
    ...
    links.split(',').map((l) => l.trim()).filter((l) => l).forEach((link) => formData.append('links', link));
    ...
    try {
      ...
      setResult(data.text || 'No output generated.');
    } catch (err) {
      console.error(err);
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        <CardContent className="space-y-6">
          ...
          {/* File Upload */}
          <div className="space-y-2">
            <label htmlFor="file-upload" className="font-medium flex items-center cursor-pointer text-blue-600">
          ...
          <div className="space-y-2">
            ...
            <select value={grade} onChange={(e) => setGrade(e.target.value)} className="border rounded px-3 py-2 w-full text-gray-900">
          ...
          {/* Process Button */}
          <Button className="w-full flex justify-center" onClick={handleProcess} disabled={loading || (!files.length && !links.trim())} >
        ...
        <Card>
          <CardContent>
            <h2 className="text-xl font-semibold mb-2 flex items-center text-gray-800">
              <FileText className="mr-2" /> Generated Output
            </h2>
            <Textarea className="h-96 text-gray-800" readOnly value={result} />
          </CardContent>
        </Card>
      )}
    </div>
